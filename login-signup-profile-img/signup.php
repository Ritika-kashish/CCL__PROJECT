    ...
    if (empty($fname)) {
       ...
       echo "Country is required";
        ...
        if(isset($_FILES['pp']['name']) AND !empty($_FILES['pp']['name'])){
           ...
           $sql = "INSERT INTO regs(fname, password, mobile_number, state, occupation, email, country, city,pp) 
                   VALUES(?,?,?,?,?,?,?,?,?)";
           $stml = $conn->prepare($sql);
           $stml->execute([$fname, $pass, $mob, $state, $occup, $email, $con, $city, $new_img_name]);
            ...
            //insert into database
         $sql = "INSERT INTO regs(fname, password, mobile_number, state, occupation, email, country, city) 
                 VALUES(?,?,?,?,?,?,?,?)";
        $stml = $conn->prepare($sql);
